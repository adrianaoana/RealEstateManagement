...
public class CustomUserDetailsService implements UserDetailsService {
	...
	/**
	 * Returns a populated {@link UserDetails} object. The email is first
	 * retrieved from the database and then mapped to a {@link UserDetails}
	 * object.
	 */
	public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {

		com.squad.model.User domainUser = userDAO.getByEmail(email);
		int userRole = 1;
		if (domainUser.getRole() != null && domainUser.getRole().contentEquals("ROLE_ADMIN")) {// admin
			userRole = 2;
		}
		...
		User user = new User(domainUser.getEmail(), domainUser.getPassword(), enabled, accountNonExpired,
